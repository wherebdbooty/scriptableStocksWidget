...
//change the widget display info. can be "price", "change", "chart"
let _showing = "price"
...
const substitutionTable = createSubstitutionTable()
...
const WW	= 342
const WH	= 317 //-30 for footer info space
...
let stocks = await webview.evaluateJavaScript(`
	let _list = document.querySelectorAll('td');
	let _results=[];
	let _t = {};
	for(let i=0; i<_list.length;i++){
		switch(_list[i].getAttribute("aria-label")){
			case "Last Price":	_t.price = _list[i].innerText//.replace(",","");                  break;
			case "Change":		_t.change = _list[i].firstChild.innerText.replace(",","");        break;
			case "Chg %":		_t.changePct = _list[i].firstChild.innerText;                     break;
			case "Day Chart":	_t.chart = _list[i].firstChild.firstChild.toDataURL('image/png'); break;
		}
...
for(let i=0;i<stockList.length;i++)
	stocks[i].name=substitutionTable[stockList[i].split("=")[0]]?substitutionTable[stockList[i].split("=")[0]]:stockList[i].split("=")[0]
